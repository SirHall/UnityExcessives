...
namespace Excessives
{
	static class MathE
	{
		...
		//Get a bit at any point
		public static bool GetBit (byte[] bitList, UInt64 position)
		{
			return
                GetBit (
				bitList [Mathf.FloorToInt (position / 8)],
				(int)position % 8
			);
		}
		...
		{
			return ((b - a) * Mathf.Sin (t * Mathf.PI / 2)) + a;
		}
		...
		public static float UnQuadLerp (float a, float b, float lerped)
		{
			...
			//Solve for t
			//t = Sqrt((lerped - a) / (b - a))
			return Mathf.Sqrt ((lerped - a) / (b - a));
		}
